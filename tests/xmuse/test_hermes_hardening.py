...
def load_hardening_module():
    module_path = Path(__file__).resolve().parents[2] / "xmuse" / "hermes_hardening.py"
...
def test_launcher_uses_stable_lockfile_without_unlinking() -> None:
    launcher = (Path(__file__).resolve().parents[2] / "xmuse" / "god_launcher.sh").read_text(
        encoding="utf-8"
    )
...
def test_hermes_gitignore_covers_runtime_control_artifacts() -> None:
    gitignore = (Path(__file__).resolve().parents[2] / "xmuse" / ".gitignore").read_text(
        encoding="utf-8"
    )
