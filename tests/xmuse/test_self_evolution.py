...
def _stored_prompt(root: Path, lane: dict[str, object]) -> str:
    prompt_ref = lane.get("prompt_ref")
    assert isinstance(prompt_ref, str)
    return (root / prompt_ref).read_text(encoding="utf-8")
            ...
            "lanes": [
                {
                    ...
                    "review_summary": (
                        ...
                        "Verification:\n"
                        "- `uv run pytest tests/xmuse/test_self_evolution.py -q` "
                        "-> `18 passed`\n"
...
def test_merge_review_confirmations_are_preserved_as_compact_evidence_signals(
    ...
    assert lane_signals == [
        {
            ...
            "review_confirmations": [
                "uv run pytest tests/xmuse/test_self_evolution.py -q -> 18 passed",
                (
                    "uv run ruff check src/xmuse_core/self_evolution/controller.py "
                    "-> All checks passed"
                ),
    ...
    assert "confirmation=uv run pytest tests/xmuse/test_self_evolution.py -q -> 18 passed" in prompt
...
def test_review_scope_refs_are_preserved_as_compact_evidence_signals(
            ...
            "lanes": [
                {
                    ...
                    "review_summary": (
                        ...
                        "tests/xmuse/test_self_evolution.py:1131).\n\n"
                        "Verification:\n"
                        "- `uv run pytest tests/xmuse/test_self_evolution.py -q` "
                        "-> `59 passed`\n\n"
    ...
    assert lane_signals[0]["review_scope_refs"] == [
        ...
        "tests/xmuse/test_self_evolution.py",
    ]
    expected = (
...
def test_bold_verification_heading_stops_review_scope_ref_collection(
    ...
    assert "reviewed=tests/xmuse/test_self_evolution.py" not in evidence.summary
    assert "reviewed=tests/xmuse/test_self_evolution.py" not in (
...
def test_merge_review_verification_run_header_is_evidence_first(
            ...
            "lanes": [
                {
                    ...
                    "review_summary": (
                        ...
                        "-> `20 passed`\n"
                        "- `uv run ruff check src/xmuse_core/self_evolution/controller.py "
                        "tests/xmuse/test_self_evolution.py` -> passed"
                    ),
                }
            ]
        },
    ...
    assert lane_signals[0]["review_confirmations"] == [
        "uv run pytest tests/xmuse/test_self_evolution.py -q -> 20 passed",
        (
            "uv run ruff check src/xmuse_core/self_evolution/controller.py "
            "tests/xmuse/test_self_evolution.py -> passed"
        ),
    ...
    assert "confirmation=uv run pytest tests/xmuse/test_self_evolution.py -q -> 20 passed" in prompt
    assert "confirmation=uv run ruff check src/xmuse_core/self_evolution/controller.py" in prompt
...
def test_multiple_merge_verifications_displace_generic_no_findings_in_signal_slots(
            ...
            "lanes": [
                {
                    ...
                    "review_summary": (
                        ...
                        "- `uv run pytest tests/xmuse/test_self_evolution.py -q` "
                        "-> `21 passed`\n"
                        "- `uv run pytest tests/xmuse/test_self_evolution_checkpoint.py -q` "
                        "-> `1 passed`\n"
                        "- `uv run ruff check src/xmuse_core/self_evolution/controller.py "
                        "tests/xmuse/test_self_evolution.py` -> passed"
                    ),
    ...
    assert lane_signals[0]["review_confirmations"] == [
        "uv run pytest tests/xmuse/test_self_evolution.py -q -> 21 passed",
        "uv run pytest tests/xmuse/test_self_evolution_checkpoint.py -q -> 1 passed",
    ]
    prompt = proposal.candidate_graph["lanes"][0]["prompt"]
    assert "confirmation=uv run pytest tests/xmuse/test_self_evolution.py -q -> 21 passed" in prompt
    assert (
        "confirmation=uv run pytest tests/xmuse/test_self_evolution_checkpoint.py -q -> 1 passed"
        in prompt
    )
...
def test_narrow_targeted_confirmation_does_not_displace_broader_merge_evidence(
            ...
            "lanes": [
                {
                    ...
                    "review_summary": (
                        "Verification:\n"
                        "- `uv run pytest tests/xmuse/test_self_evolution.py::test_small -q` "
                        "-> `2 passed`\n"
                        "- `uv run ruff check src/xmuse_core/self_evolution/controller.py "
                        "tests/xmuse/test_self_evolution.py` -> passed\n"
                        "- `uv run pytest tests/xmuse/test_self_evolution.py "
                        "tests/xmuse/test_self_evolution_checkpoint.py -q` -> `38 passed`"
                    ),
                }
            ]
        },
    ...
    assert "confirmation=uv run ruff check src/xmuse_core/self_evolution/controller.py" in prompt
    assert (
        "confirmation=uv run pytest tests/xmuse/test_self_evolution.py "
        "tests/xmuse/test_self_evolution_checkpoint.py -q -> 38 passed"
        in prompt
    )
            ...
            "lanes": [
                {
                    ...
                    "review_summary": (
                        ...
                        "passed\n"
                        "- `uv run pytest tests/xmuse/test_self_evolution.py "
                        "tests/xmuse/test_self_evolution_checkpoint.py -q` passed: 42 tests\n"
...
def test_result_line_confirmation_preserves_preceding_verification_command(
    ...
    assert "confirmation=uv run pytest tests/xmuse/test_self_evolution.py::" in prompt
...
def test_review_residual_risk_is_preserved_as_evidence_signal(
    ...
    assert "confirmation=uv run pytest tests/xmuse/test_self_evolution.py -q -> 42 passed" in prompt
...
def test_findings_residual_risk_examples_do_not_become_review_risks(
            ...
            "lanes": [
                {
                    ...
                    "review_summary": (
                        ...
                        "Verification:\n"
                        "- `uv run pytest tests/xmuse/test_self_evolution.py "
                        "tests/xmuse/test_self_evolution_checkpoint.py -q` "
                        "-> `50 passed`"
                    ),
                }
            ]
    ...
    assert lane_signals[0]["review_confirmations"] == [
        (
            "uv run pytest tests/xmuse/test_self_evolution.py "
            "tests/xmuse/test_self_evolution_checkpoint.py -q -> 50 passed"
        )
    ]
    prompt = proposal.candidate_graph["lanes"][0]["prompt"]
    assert "risk=I did not run the full suite" not in prompt
...
def test_multiple_merge_confirmations_are_delimited_in_evidence_summary(
    ...
    prompt = proposal.candidate_graph["lanes"][0]["prompt"]
    assert (
        "confirmation=uv run pytest tests/xmuse/test_self_evolution.py -q -> 22 passed; "
        "confirmation=uv run pytest tests/xmuse/test_self_evolution_checkpoint.py -q -> 1 passed"
        in evidence.summary
    )
    assert (
...
def test_gate_report_ref_is_promoted_to_compact_evidence_signal(
    ...
    _write_json(
        tmp_path / report_ref,
        {
            "status": "passed",
            "commands": [
                {"command": "uv run pytest tests/xmuse/test_self_evolution.py -q"}
            ],
        },
    )
    controller = SelfEvolutionController(
...
def test_legacy_gate_report_status_and_commands_become_evidence_signals(
    ...
    expected_command = (
        "gate_command=uv run pytest tests/xmuse/test_self_evolution.py -q -> passed"
    )
...
def test_gate_report_command_result_is_promoted_to_evidence_signal(
    ...
    expected = (
        "gate_command=uv run pytest -q tests/xmuse/test_self_evolution.py "
...
def test_long_pytest_gate_command_summarizes_target_count_without_truncation(
    ...
    test_targets = [
        "tests/xmuse/test_quality_gate.py",
        "tests/xmuse/test_gate_profiles.py",
        "tests/xmuse/test_master_loop.py",
        "tests/xmuse/test_master_loop_integration.py",
        "tests/xmuse/test_auto_discovery.py",
        "tests/xmuse/test_rework_loop.py",
        "tests/xmuse/test_core_agents_consumer.py",
        "tests/xmuse/test_core_agents_launchers.py",
        "tests/xmuse/test_core_agents_manager.py",
        "tests/xmuse/test_core_agents_protocol.py",
        "tests/xmuse/test_core_agents_registry.py",
        "tests/xmuse/test_core_agents_session.py",
        "tests/xmuse/test_core_schema.py",
        "tests/xmuse/test_core_state.py",
        "tests/xmuse/test_core_status.py",
        "tests/xmuse/test_core_routing.py",
        "tests/xmuse/test_core_callback_server.py",
        "tests/xmuse/test_mcp_server.py",
        "tests/xmuse/test_overnight_runner.py",
        "tests/xmuse/test_self_evolution.py",
        "tests/xmuse/test_self_evolution_checkpoint.py",
    ]
    _write_json(
        tmp_path / report_ref,
        {
    ...
    expected = (
        "gate_command=uv run pytest -q tests/xmuse/test_quality_gate.py "
        "tests/xmuse/test_gate_profiles.py +19 test files -> passed "
...
def test_gate_report_diagnostics_reach_evidence_prompt(tmp_path: Path) -> None:
    ...
    assert expected in proposal.candidate_graph["lanes"][0]["prompt"]
    assert (
        "gate_command=uv run pytest -q tests/xmuse/test_gate_profiles.py -> passed"
        in evidence.summary
    )
...
def test_gate_report_signal_remains_visible_with_many_lane_signals(
    ...
    assert (
        "gate_command=uv run pytest tests/xmuse/test_self_evolution.py -q "
...
def test_candidate_prompt_preserves_long_confirmation_outcome(
    ...
    assert "confirmation=uv run pytest tests/xmuse/test_self_evolution.py -q -> 25 passed" in prompt
...
def test_dry_run_lands_visible_conversation_resolution_graph_and_lineage(tmp_path: Path) -> None:
    ...
    assert spawned[0]["blueprint_refs"] == ["blueprint.md"]
...
def test_run_from_existing_evidence_bundle_preserves_bundle_identity(tmp_path: Path) -> None:
    ...
    lanes = json.loads((tmp_path / "feature_lanes.json").read_text(encoding="utf-8"))["lanes"]
    spawned = [lane for lane in lanes if lane.get("resolution_id") == lineage.spawned_resolution_id]
    assert "prompt" not in spawned[0]
    assert _stored_prompt(tmp_path, spawned[0]).count(evidence.bundle_id) == 1
...
def test_run_from_stale_merged_evidence_bundle_hydrates_review_confirmations(
    ...
    assert (
        "confirmation=uv run pytest tests/xmuse/test_self_evolution.py -q -> 18 passed"
        in hydrated.summary
    )
    ...
    assert len(spawned) == 1
    assert "prompt" not in spawned[0]
    prompt = _stored_prompt(tmp_path, spawned[0])
    assert "status=merged review=merge gate=passed retries=1" in prompt
    assert (
        "confirmation=uv run pytest tests/xmuse/test_self_evolution.py -q -> 18 passed"
        in prompt
    )
            ...
            "command_results": [
                {
                    "command_id": "pytest",
                    "profile_id": "xmuse-core",
                    "argv": ["uv", "run", "pytest", "-q", "tests/xmuse/test_self_evolution.py"],
                    "returncode": 0,
                    "stdout_path": str(stdout_path),
                }
            ],
    ...
    for lane in lanes_data["lanes"]:
        if lane.get("feature_id") == lane_id:
            lane["review_summary"] = (
                ...
                "(/repo/tests/xmuse/test_self_evolution.py:620).\n\n"
                f"{lane['review_summary']}"
            )
    lanes_path.write_text(json.dumps(lanes_data, indent=2) + "\n", encoding="utf-8")
