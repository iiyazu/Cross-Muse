...
def test_create_conversation_with_initial_participants_uses_requested_set(
    ...
    assert participants[0]["model"] == "gpt-5.4"
    assert participants[0]["role_template_id"] == review_template["id"]


def test_create_conversation_rejects_claude_initial_participant(
    tmp_path: Path,
) -> None:
    client = _client(tmp_path)
...
def test_delete_role_template_rejects_existing_participant_references(
    tmp_path: Path,
) -> None:
    client = _client(tmp_path)
    ...
    assert (
        client.get("/api/chat/role-templates").json()["role_templates"][-1]["id"]
        == template["id"]
    )
...
def test_role_template_text_fields_reject_whitespace_only(tmp_path: Path) -> None:
    client = _client(tmp_path)
...
def test_participant_text_fields_reject_whitespace_only(tmp_path: Path) -> None:
    client = _client(tmp_path)
    conversation = client.post(
        "/api/chat/conversations",
        json={"title": "participant validation", "initial_participants": []},
    ).json()
