...
def test_stable_verdict_id_uses_review_task_id(tmp_path: Path) -> None:
    """stable_verdict_id_for_lane uses review_task_id when available."""
    orch = _make_orchestrator(
        tmp_path,
        [_gated_lane("lane-stable-id", review_task_id="rtask_abc123")],
    )

    verdict_id = stable_verdict_id_for_lane(
        "lane-stable-id",
        lane=orch._sm.get_lane("lane-stable-id"),
    )
...
def test_stable_verdict_id_falls_back_to_lane_id(tmp_path: Path) -> None:
    """stable_verdict_id_for_lane falls back to lane_id when no task_id."""
    orch = _make_orchestrator(tmp_path, [_gated_lane("lane-no-task-id")])

    verdict_id = stable_verdict_id_for_lane(
        "lane-no-task-id",
        lane=orch._sm.get_lane("lane-no-task-id"),
    )
...
def test_stable_verdict_id_is_deterministic(tmp_path: Path) -> None:
    """stable_verdict_id_for_lane returns the same value on repeated calls."""
    orch = _make_orchestrator(
        tmp_path,
        [_gated_lane("lane-det", review_task_id="rtask_det")],
    )

    lane = orch._sm.get_lane("lane-det")
    id1 = stable_verdict_id_for_lane("lane-det", lane=lane)
    id2 = stable_verdict_id_for_lane("lane-det", lane=lane)
...
def test_ingest_merge_verdict_for_lane_is_noop_when_no_task_id(tmp_path: Path) -> None:
    """ingest_merge_verdict is a no-op when review_task_id is absent."""
    orch = _make_orchestrator(
        tmp_path,
        [
            {
    ...
    ingest_merge_verdict(
        "lane-no-task-merge",
        "Approved.",
        lane=orch._sm.get_lane("lane-no-task-merge"),
...
def test_ingest_merge_verdict_for_lane_persists_verdict(tmp_path: Path) -> None:
    """ingest_merge_verdict persists a merge verdict in the store."""
    orch = _make_orchestrator(
        tmp_path,
    ...
    ingest_merge_verdict(
        "lane-ingest-merge",
        "Approved.",
        lane=orch._sm.get_lane("lane-ingest-merge"),
        review_plane=orch._review_plane,
    )
