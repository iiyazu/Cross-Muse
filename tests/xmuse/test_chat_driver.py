...
def _make_driver(
    ...
    """Build a ChatDriver whose Codex subprocess returns canned outputs.

    Returns the driver plus a list that records each call's prompt for assertions.
    """
    db = tmp_path / "chat.db"
    driver = ChatDriver(chat_db_path=db, runtime="codex", model="gpt-5.4", timeout_s=5.0)
    calls: list[dict] = []
    queue = list(codex_outputs)

    def fake_run(cmd, input, capture_output, text, timeout, check):  # noqa: ARG001
        if not queue:
            raise AssertionError("codex called more times than canned outputs")
        out = queue.pop(0)
        calls.append({"input": input, "cmd": cmd})
        return _Stub(stdout=out, stderr="", returncode=0)
...
def test_driver_rejects_claude_runtime(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="codex-only"):
        ChatDriver(chat_db_path=tmp_path / "chat.db", runtime="claude")


def test_driver_ignores_claude_runtime_env(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("XMUSE_CHAT_DRIVER_RUNTIME", "claude")
    driver = ChatDriver(chat_db_path=tmp_path / "chat.db")

    assert driver._runtime == "codex"


def test_driver_ignores_legacy_non_god_model_env(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("XMUSE_NON_GOD_CODEX_MODEL", "gpt-5.4")
    monkeypatch.delenv("XMUSE_CODEX_MODEL", raising=False)

    driver = ChatDriver(chat_db_path=tmp_path / "chat.db")

    assert driver._model == "gpt-5.4"


def test_driver_normalizes_reserved_final_quality_model_env(
    tmp_path: Path,
    monkeypatch,
) -> None:
...
def test_driver_routes_default_to_architect(tmp_path: Path, monkeypatch) -> None:
    ...
    driver.tick()  # primes cursor, no codex call expected to consume queue
    assert len(calls) == 0
...
def test_driver_skips_assistant_role_messages(tmp_path: Path, monkeypatch) -> None:
    ...
    # message (it should be skipped without spawning Codex).
    assert outcomes == []
