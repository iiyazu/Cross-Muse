...
def require_legacy_runtime_assets(*relative_paths: str) -> None:
    missing = [path for path in relative_paths if not (PROJECT / path).exists()]
    if missing:
        pytest.skip(
            "legacy runtime assets are intentionally excluded from standalone xmuse export: "
            + ", ".join(missing)
        )
...
def test_xmuse_console_is_not_control_plane_merge_quarantined():
    require_legacy_runtime_assets("xmuse/master_state.json")
    master_state = json.loads((PROJECT / "xmuse" / "master_state.json").read_text())
    feature = next(
