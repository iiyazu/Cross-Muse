...
def test_transition_increments_projection_revision(lanes_path):
...
def test_transition_appends_state_history_snapshot(tmp_path):
    lanes_path = tmp_path / "feature_lanes.json"
    lanes_path.write_text(json.dumps({"lanes": [
        {"feature_id": "lane-1", "status": "pending", "prompt": "fix bug"},
    ]}))
    history_path = tmp_path / "state_history.json"
    history_sm = LaneStateMachine(lanes_path, history_path=history_path)

    history_sm.transition(
        "lane-1",
        "dispatched",
        metadata={"worker_pid": 123},
    )
    ...
    assert snapshot["state_key"] == "dispatched"
    assert snapshot["metadata"]["from_status"] == "pending"
    assert snapshot["metadata"]["worker_pid"] == 123
...
def test_conditional_transition_requires_expected_metadata(tmp_path):
    path, sm = _make_sm(tmp_path, [
        {
            "feature_id": "lane-1",
            "status": "dispatched",
    ...
    result = sm.transition_if_metadata(
        "lane-1",
        "exec_failed",
        expected_metadata={"status": "dispatched", "worker_pid": 123},
        metadata={"failure_reason": "stale_worker_lost"},
    )

    assert result is None
    lane = sm.get_lane("lane-1")
    assert lane["status"] == "dispatched"
    assert lane["worker_pid"] == 456
    assert "projection_revision" not in json.loads(path.read_text(encoding="utf-8"))


def test_conditional_transition_applies_when_expected_metadata_matches(tmp_path):
    _, sm = _make_sm(tmp_path, [
        {
            "feature_id": "lane-1",
            "status": "dispatched",
    ...
    assert result["status"] == "exec_failed"
    assert result["failure_reason"] == "stale_worker_lost"


def test_transition_guard_rejects_stale_projection_revision_without_write(tmp_path):
    path = tmp_path / "feature_lanes.json"
    before = {
    ...
    def require_revision(
        ...
        if state.get("projection_revision") != 3:
            raise InvalidTransitionError("stale projection revision")

    with pytest.raises(InvalidTransitionError, match="stale projection revision"):
        sm.transition("lane-1", "dispatched", guard=require_revision)
...
def test_update_metadata_drops_runtime_telemetry_fields(tmp_path):
    path, sm = _make_sm(tmp_path, [
        {"feature_id": "lane-1", "status": "pending", "prompt": "p"},
    ])
    ...
    sm.update_metadata(
        ...
        {
            "tag": "hotfix",
            "recovery_events": [{"kind": "retry_scheduled"}],
            "last_recovery_event": {"kind": "retry_scheduled"},
            "failure_error": "large transient stderr",
        },
    )

    lane = sm.get_lane("lane-1")
    assert lane["tag"] == "hotfix"
    assert "recovery_events" not in lane
    assert "last_recovery_event" not in lane
    assert "failure_error" not in lane


def test_transition_metadata_drops_runtime_telemetry_fields(tmp_path):
    _, sm = _make_sm(tmp_path, [
        {"feature_id": "lane-1", "status": "dispatched", "prompt": "p"},
    ])
    ...
    sm.transition(
        ...
        "exec_failed",
        metadata={
            "failure_reason": "execution_spawn_failed",
            "failure_error": "large transient stderr",
            "recovery_events": [{"kind": "operation_failed"}],
            "last_recovery_event": {"kind": "operation_failed"},
        },
    )
...
def test_append_lane_drops_runtime_telemetry_fields(tmp_path):
    _, sm = _make_sm(tmp_path, [])
...
def test_append_lane_persists_to_file(tmp_path):
    path, sm = _make_sm(tmp_path, [])
    returned = sm.append_lane({"feature_id": "lane-new", "status": "pending", "prompt": "x"})
    data = json.loads(path.read_text())
    assert len(data["lanes"]) == 1
    assert data["lanes"][0]["feature_id"] == "lane-new"
    assert data["projection_revision"] == 1
    assert "projection_revision" not in returned


def test_duplicate_append_lane_does_not_increment_projection_revision(tmp_path):
    path, sm = _make_sm(tmp_path, [
        {"feature_id": "lane-1", "status": "pending", "prompt": "p"},
    ])
    sm.append_lane({"feature_id": "lane-2", "status": "pending", "prompt": "q"})
    sm.append_lane({"feature_id": "lane-2", "status": "dispatched", "prompt": "dup"})

    data = json.loads(path.read_text(encoding="utf-8"))
    assert data["projection_revision"] == 1
    assert [lane["feature_id"] for lane in data["lanes"]] == ["lane-1", "lane-2"]
