...
def test_update_lane_status_uses_projection_revision(tmp_path):
    from master_loop import update_lane_status

    lanes_path = tmp_path / "feature_lanes.json"
    _write_lanes(lanes_path, [{"feature_id": "lane-1", "status": "pending"}])

    update_lane_status(lanes_path, "lane-1", "running")

    data = _read_doc(lanes_path)
    assert data["projection_revision"] == 1
    assert data["lanes"][0]["status"] == "running"
...
@pytest.mark.asyncio
async def test_review_gate_exception_blocks_merge(tmp_path):
    from master_loop import MasterLoop
    ...
    lane = _read_lanes(lanes_path)[0]
    verdict = lane["review_verdict"]
    assert summary.failed_lanes == 1
    assert lane["status"] == "failed"
    assert [task.task_type for task in loop.consumer.dispatched] == ["execute"]
    assert verdict["approved"] is False
    assert verdict["confidence"] == 0.0
    assert verdict["summary"] == "review gate unavailable, blocking merge"
...
async def test_failed_full_quality_gate_generates_priority_repair_lane(
    ...
    assert "prompt" not in repair
    prompt = _stored_prompt(tmp_path, repair)
    assert "failed test output" in prompt
    assert "Profile: strict-product" in prompt
