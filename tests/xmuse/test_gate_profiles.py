...
def _loaded_config(tmp_path: Path):
    ...
    data["profiles"]["xmuse-core"] = {
        ...
        "env": {},
        "commands": [
            {"command": "pytest", "args": ["-q", "tests/xmuse/test_master_loop.py"]}
        ],
        "diff_selectors": ["xmuse/**", "src/xmuse_core/**"],
        "test_files": ["tests/xmuse/test_master_loop.py"],
        "test_nodeids": [],
        "test_markers": [],
        "mixed_test_files": [],
    }
...
@pytest.mark.asyncio
async def test_gate_runner_keeps_missing_pytest_file_args_for_blocking_commands(
    tmp_path, monkeypatch
):
    ...
    async def fake_exec(*args, **kwargs):
        calls.append(args)
        if "tests/xmuse/test_overnight_runner.py" in args:
            return FakeProcess(stderr="missing", returncode=4)
        return FakeProcess(stdout="ok", returncode=0)
...
def test_xmuse_core_gate_runs_peer_chat_regression_tests():
    ...
    assert "tests/xmuse/test_peer_chat_service.py" in command_args
    assert "tests/xmuse/test_peer_chat_service.py" in profile.test_files
    assert "tests/xmuse/test_peer_chat_dashboard.py" in command_args
    assert "tests/xmuse/test_peer_chat_dashboard.py" in profile.test_files
    assert "tests/xmuse/test_peer_chat_api.py" in command_args
    assert "tests/xmuse/test_peer_chat_api.py" in profile.test_files
    assert "tests/xmuse/test_peer_chat_mcp_tools.py" in command_args
    assert "tests/xmuse/test_peer_chat_mcp_tools.py" in profile.test_files
    assert "tests/xmuse/test_peer_chat_turn_budget.py" in command_args
    assert "tests/xmuse/test_peer_chat_turn_budget.py" in profile.test_files
    assert "tests/xmuse/test_peer_chat_scheduler.py" in command_args
    assert "tests/xmuse/test_peer_chat_scheduler.py" in profile.test_files
    assert "tests/xmuse/test_peer_chat_end_to_end.py" in command_args
    assert "tests/xmuse/test_peer_chat_end_to_end.py" in profile.test_files
    assert "tests/xmuse/test_god_session_layer.py" in command_args
    assert "tests/xmuse/test_god_session_layer.py" in profile.test_files
    assert "tests/xmuse/test_platform_runner.py" in command_args
...
def test_xmuse_core_gate_runs_b4_feature_graph_tests():
    ...
    required_test_files = {
        "tests/xmuse/test_feature_plan_graph_set.py",
    ...
    for test_file in required_test_files:
        ...
        assert test_file in command_args
        assert test_file in profile.test_files


def test_xmuse_core_gate_runs_b4_feature_summary_after_lane_five_exists():
    config = load_gate_config(Path("xmuse/gate_profiles.json"), repo_root=Path("."))
    profile = config.profiles["xmuse-core"]
    ...
    test_file = "tests/xmuse/test_feature_summary.py"
