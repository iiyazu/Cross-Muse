...
| Area | Session-4 delta |
|---|---|
| Frontdoor | 当前用户前门方向是 Textual TUI；旧 `xmuse/frontend/` browser frontend 已归档 |
| Runtime assumption | 不假设当前有 live runner；API 文档描述可用 surface，不代表后台正在运行 |
| Architecture contract | 新增 `docs/xmuse/解耦开发协议.md`，明确 TUI、GOD runtime、workflow、dashboard 边界 |
| Dashboard role | Dashboard 保持灰盒 drill-down，不作为主入口或 workflow 驱动器 |
| State authority | `feature_lanes.json` 继续视为 legacy projection/syncer，不是权威图 |

## What changed since session 2

| Area | Session-3 delta |
|---|---|
...
| Dashboard peer chat | dashboard peer-chat conversation/session/request/lane-graph/feature-graph-set/execution-card drill-down endpoints live |
| C-class | graph-set 已注入并运行；PlanningRun/card/API 仍未成为稳定前端契约 |

## Session-3 Frontend Development Guidance

推荐先实现一个 chat-first MVP：
...
不要在前端直接读写 `xmuse/feature_lanes.json` 或 runtime JSON 文件。

## Session-3 Missing Endpoints To Treat As Placeholders
...
- C-class PlanningRun endpoints
- approved blueprint -> feature plan -> lane graph autonomous execution cards
...
| GOD runtime | 当前运行时为 Codex-only，默认模型 `gpt-5.5`；旧 Claude 配置不再作为可选前端状态展示 |
...
## GOD Runtime Selection (session 2 update)

The runner and chat plane are currently Codex-only. Execute/review GODs,
chat-driver GOD replies, and peer-chat decomposition default to `gpt-5.5`.
Legacy lane metadata or old local environment variables naming Claude should
not be surfaced as an active runtime choice.

Frontend impact: none for now. A future `/api/runtime/info` should return
`codex` plus the active model so the dashboard can label sessions correctly.
