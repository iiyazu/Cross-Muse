...
> 2026-06-02 HKT 更新：本文是早期 Open Design/browser 前端上下文，部分内容已经过时。
> 当前独立前端开发入口请优先阅读 `docs/xmuse/frontend/FRONTEND_IMPLEMENTATION_GUIDE.md`、
> `docs/xmuse/frontend/FRONTEND_API.md`、`docs/xmuse/frontend/FRONTEND_API_INCREMENTAL.md`、
> `docs/xmuse/README.md` 和 `docs/xmuse/解耦开发协议.md`。
> 当前任务只基于 WSL/Linux 侧 xmuse 文档和代码，不依赖 Windows/Open Design
> 前端目录。
> 当前用户前门方向是 Textual TUI；旧 `xmuse/frontend/` browser frontend 已归档。

本文档保留为历史背景，描述 xmuse 系统的后端能力、数据流和交互需求。新开发应
优先遵循 Textual TUI + compact cards + dashboard drill-down 的路径。

## 系统概述

xmuse 是一个本地多 agent 自治编排系统，目标是由 Ray 管理持久化 GOD 群聊，
再由原生 workflow 或 LangGraph 将 approved blueprint 拆成 feature / lane graph
并并行执行。TUI 需要展示群聊、compact cards 和 worklist 摘要；dashboard 保持
灰盒 drill-down。
...
2026-05-31 现状：xmuse 已有 FastAPI REST surface，前端不应直接读取文件。
下面表格保留为历史背景；当前接口以 `docs/xmuse/frontend/FRONTEND_API.md` 和
`docs/xmuse/frontend/FRONTEND_IMPLEMENTATION_GUIDE.md` 为准。
...
- Conversation/workspace 列表。
- 人类和 GOD/peer 的对等消息流。
- Compact cards: blueprint、feature plan、lane graph、run progress、review
  verdict、takeover。
- Worklist 摘要；点击后进入 dashboard drill-down。

### 2. 调度监控面板

- Lane 状态总览（done/running/pending/failed 计数 + 列表）
- 实时 lane 状态变化（优先 read envelope/API polling，不直接读写 `feature_lanes.json`）
- Lane 依赖图可视化（DAG，显示并发组）
- 点击 lane 查看详情（prompt、产出、日志、review verdict）

### 3. God Group Chat（长 session 交互，核心新功能）
