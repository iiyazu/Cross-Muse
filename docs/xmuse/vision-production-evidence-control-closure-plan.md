...
- The same release pack command can accept `--github-server-truth`, convert the
  ...
  `proof_level_summary` from the readiness report for handoff inspection,
  including attempted command and next-action recovery fields; this does not
  replace the readiness report or gate artifacts.
- The TUI proof cockpit can project the same raw GitHub server truth boundary
  ...
  without upgrading the release gate or emitting `pr_merged`.
