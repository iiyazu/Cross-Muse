...
| DR03-DEBT-003 | `src/xmuse_core/integrations/memoryos_lite_interop.py` | `XMUSE_LIVE_MEMORYOS_LITE=1 XMUSE_MEMORYOS_LITE_URL=http://127.0.0.1:8000 uv run pytest -q tests/xmuse/test_memoryos_lite_interop.py` | Default tests prove fake REST contract only; live MemoryOS Lite compatibility is opt-in runtime proof. | P0 | A live service check is added to an operator-run workflow or release checklist with captured evidence. |
