...
from xmuse_core.chat.review_triggers import (
    ReviewTriggerService,
    proposal_id_from_message,
)
...
class PeerChatService:
        ...
        *,
        ...
        if reply_to_inbox_item_id:
            ...
            if isinstance(message_id, str) and message_id:
                self._record_review_trigger_reply(
                    conversation_id=conversation_id,
                    participant_id=participant_id,
                    inbox_item_id=reply_to_inbox_item_id,
    ...
    def _record_review_trigger_reply(
        self,
        *,
        conversation_id: str,
        participant_id: str,
        ...
        if (
            ...
            or item.target_participant_id != participant_id
            or item.item_type != "review_trigger"
        ):
            return
    ...
    def _attach_acceptance_spine_proposal_for_inbox_reply(
        ...
        *,
        ...
        AcceptanceSpineStore(self._db_path).attach_proposal_for_inbox_reply(
            conversation_id=conversation_id,
            inbox_item_id=inbox_item_id,
            proposal_id=proposal_id,
        )
    ...
    def _ensure_review_trigger(
        ...
        *,
        ...
        ReviewTriggerService(self._db_path).ensure_for_message(
            conversation_id=conversation_id,
...
def classify_review_trigger_reply(content: str) -> str:
    clean = content.strip()
    if not clean:
        return "unknown"
    normalized = " ".join(clean.lower().replace("_", " ").split())
    first_line = " ".join(clean.splitlines()[0].lower().replace("_", " ").split())
    if (
        "review verdict: blocked" in first_line
        or "review verdict blocked" in first_line
        or "blocks dispatch=true" in normalized
        or "blocks_dispatch=true" in clean.lower()
    ):
        return "blocked"
    if "review verdict" in first_line and any(
