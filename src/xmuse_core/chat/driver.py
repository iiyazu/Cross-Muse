...
new human messages, routes them by @mention, spawns a one-shot Codex
subprocess for the matching role, and writes the GOD's reply back into
the conversation.

Runtime is codex-only. ``XMUSE_CHAT_DRIVER_RUNTIME`` is ignored to prevent
stale environments from switching xmuse back to Claude.
...
from xmuse_core.chat.mentions import DEFAULT_INTAKE_ROLE
    ...
    def __init__(
        ...
        *,
        ...
        if requested_runtime != "codex":
            raise ValueError(f"ChatDriver runtime must be codex-only, got {requested_runtime!r}")
        self._runtime = "codex"
        self._model = normalize_codex_model_id(
            model
            if model is not None
            else os.environ.get("XMUSE_CODEX_MODEL", DEFAULT_CODEX_GOD_MODEL_ID),
