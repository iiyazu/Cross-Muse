"""Framework-neutral recovery primitives owned by xmuse."""
...
class RecoveryConfig:
    ...
    def from_env(cls, prefix: str = "XMUSE_RECOVERY") -> RecoveryConfig:
        return cls(
            ...
            initial_delay_s=_env_float(
                f"{prefix}_INITIAL_DELAY_S", cls.initial_delay_s
            ),
    ...
    def normalized(self) -> RecoveryConfig:
        return RecoveryConfig(
            ...
            circuit_recovery_timeout_s=max(
                0.0, float(self.circuit_recovery_timeout_s)
            ),
...
class RecoveryManager:
    ...
    def execute(
        ...
        *,
        ...
        is_transient: Callable[
            [BaseException], bool
        ] = lambda exc: is_transient_failure(exc),
        observer: RecoveryObserver | None = None,
    ) -> T:
        ...
        while True:
            ...
            try:
                ...
                self._emit_failure(
                    component, operation, attempt, exc, breaker, opened, observer
                )
                ...
                self._emit_retry(
                    component, operation, attempt, delay, exc, breaker, observer
                )
                if delay > 0:
                    self._sleep(delay)
                attempt += 1
    ...
    def _emit(
        self, event: RecoveryEvent, observer: RecoveryObserver | None = None
    ) -> None:
