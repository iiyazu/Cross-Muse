...
def reconcile_closure(
    *,
    ...
    conditions = (
        recovery_present.with_observed_ref(current.recovery_artifact_ref),
        recovery_allows_progress.with_observed_ref(current.recovery_artifact_ref),
        candidate_present,
        review_present.with_observed_ref(current.review_closure_ref),
        release_evaluated.with_observed_ref(current.release_handoff_ref),
        server_pending,
    )
...
class _ConditionBuilder:
    ...
    def with_observed_ref(self, observed_ref: str | None) -> ClosureCondition:
...
def _recovery_artifact_present(
    ...
    if _text(recovery_artifact.get("schema_version")) != RECOVERY_PROOF_SCHEMA_VERSION:
        return _condition(
            RECOVERY_ARTIFACT_PRESENT,
            "false",
            "blocked",
